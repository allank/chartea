...
- `Model.VolumeFormatter` (`func(float64) string`), if set, overrides `VolumePrecision`-based volume formatting entirely — lets callers plug in their own formatting (e.g. [go-humanize](https://github.com/dustin/go-humanize)) without chartea depending on it directly. See [README: Volume formatting](README.md#volume-formatting).
